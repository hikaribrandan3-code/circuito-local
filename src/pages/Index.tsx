...
const CATEGORY_IMAGES: Record<string, string> = {
  flores: "https://images.unsplash.com/photo-1550008319-5ef12c9b8c00?auto=format&fit=crop&w=700&q=80",
    ...
    <div>
      ...
      <section className="mx-auto max-w-7xl px-4 lg:px-8 py-16 md:py-24">
        <div className="relative overflow-hidden rounded-3xl bg-foreground text-background p-10 md:p-16">
          <div className="absolute -right-20 -top-20 h-72 w-72 rounded-full bg-celeste/20 blur-3xl" />
          <div className="absolute -left-10 -bottom-10 h-60 w-60 rounded-full bg-gold/20 blur-3xl" />
          <div className="relative grid md:grid-cols-2 gap-8 items-center">
            <div>
              <span className="text-xs uppercase tracking-[0.2em] text-background/60">¿Algo a medida?</span>
              <h2 className="mt-3 font-display text-4xl md:text-5xl leading-tight">
                Armamos tu regalo <span className="serif-italic">a pedido.</span>
              </h2>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <Button asChild size="lg" className="bg-background text-foreground hover:bg-background/90 font-medium rounded-full">
                  <Link to="/contacto">Contactar <ArrowRight className="ml-1 h-4 w-4" /></Link>
                </Button>
                <Button asChild size="lg" variant="outline" className="rounded-full border-background/30 text-background hover:bg-background/10 hover:text-background">
            ...
            <ul className="grid gap-3">
              {[
                ...
                <li key={t} className="flex items-center gap-3 rounded-xl bg-background/5 border border-background/10 p-4">
                  <span className="flex h-7 w-7 items-center justify-center rounded-full bg-gold text-foreground text-xs font-semibold">✓</span>
